async function selectProduct(connection, category) {
    var selectUserListQuery = `
    SELECT  productId, B.name as brand, P.name,  price, P.image
    FROM Product P
    INNER JOIN Brand B ON P.brandId = B.brandId
    `

    if (category) {
        selectUserListQuery += ` WHERE P.category = ${category};`;
    }
...
async function selectLike(connection, userId, productId) {
    const selectLikeQuery = `
        SELECT * FROM ProductLike WHERE userId = ? AND productId = ? ;
        `;
...
async function deleteLike(connection, userId, productId) {
    const selectLikeQuery = `
        DELETE FROM ProductLike WHERE userId = ? AND productId = ? ;
        `;
    const LikeRows = await connection.query(selectLikeQuery, [userId, productId]);
    return LikeRows;
}
